...
class AutoHarnessDisableServiceLike(Protocol):
...
def _build_disable_approval_request(
    *,
    tool_name: str,
    code_hash: str,
    ...
    return ToolApprovalRequest(
        ...
        target_summary=f"Disable generated role tool {tool_name}",
        source="auto_harness",
        execution_surface=ExecutionSurface.API,
        cache_key=f"auto_harness:disable:{tool_name}:{normalized_hash}:{target_cache_key}",
        metadata={
            "tool_name": tool_name,
...
async def _run_disable_action(
    ...
    *,
    ...
    service = cast(AutoHarnessDisableServiceLike, raw_service)
    result = await service.disable_tool(
...
def register(agent: Agent[ToolDeps, str]) -> None:
    ...
    async def auto_harness_disable_tool(
        ctx: ToolContext,
        tool_name: str,
        code_hash: str,
        target_role_id: str | None = None,
    ) -> dict[str, JsonValue]:
        """Disable an enabled generated role tool after approval."""

        action = partial(_run_disable_action, ctx)
        return await execute_tool_call(
            ctx,
            tool_name=AUTO_HARNESS_DISABLE_TOOL,
            args_summary={
                "tool_name": tool_name,
                "code_hash": code_hash,
                "target_role_id": target_role_id,
            ...
            action=action,
            raw_args=locals(),
            approval_request_factory=lambda tool_input: _build_disable_approval_request(

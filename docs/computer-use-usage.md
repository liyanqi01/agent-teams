...
- 同时支持 `vm_http` 远程执行器和 `local_desktop` 本地桌面执行器

如果没有配置 executor backend，系统会回退到 `UnavailableComputerExecutor`，computer use run 不会执行真实动作。
...
- `vm_http` 和 `local_desktop` 两种 executor backend 选择与装配
...
3. `ComputerUseSession` 根据 executor backend 启动一个 computer session。
...
当前 Web 设置页已经支持直接填写 `computer_use` 参数：
...
当前 `local_desktop` 的限制和行为：

- 只支持 Windows 宿主机
- 直接使用宿主机前台桌面执行鼠标和键盘动作
- 不需要额外 VM agent
- `current_url` 目前不会自动解析，默认返回 `null`

如果没有设置 `AGENT_TEAMS_COMPUTER_EXECUTOR_BACKEND`，backend 会保持为 `unavailable`。
...
1. 在 Windows 宿主机上设置 `AGENT_TEAMS_COMPUTER_EXECUTOR_BACKEND=local_desktop`。
2. 重启 Agent Teams server。
3. 使用一个指向 computer-use profile 的 role 创建 run。
4. 保持宿主机桌面处于解锁、可见、可交互状态。
5. 审批通过后，动作会直接作用到当前桌面。
...
对于 `local_desktop` backend，审批尤其重要，因为动作会直接作用到当前宿主机桌面。
...
- 是否设置了 `AGENT_TEAMS_COMPUTER_EXECUTOR_BACKEND`
- 使用 `vm_http` 时，`AGENT_TEAMS_COMPUTER_VM_BASE_URL` 是否可达
- 使用 `vm_http` 时，VM agent 是否实现了所有必需接口
- 使用 `local_desktop` 时，当前宿主机是否为 Windows 且桌面未锁屏
...
- `local_desktop` 目前只支持 Windows
- `local_desktop` 直接控制宿主机，风险高于 VM
- `local_desktop` 不会自动提取浏览器 URL
- 没有专门的 screenshot timeline 页面
- 没有面向 `computer_sessions` / `computer_turns` 的专用 replay API
- 这条路径不支持 browser-DOM automation 模式
- 使用 `vm_http` 时仍依赖外部 VM HTTP agent

## 13. 建议的下一步

如果要继续增强这套实现，优先级最高的方向是：

1. 为 `local_desktop` 增加更细粒度的宿主机风险策略。
2. 基于 `computer_sessions` 和 `computer_turns` 增加 replay/read API。
3. 增强 screenshot timeline UI。
4. 对 `local_desktop` 增加更多平台支持。
5. 对 submit、upload、download、login 等动作增加更细粒度的风险策略。
